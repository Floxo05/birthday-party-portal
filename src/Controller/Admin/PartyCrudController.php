...
class PartyCrudController extends AbstractCrudController
{
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        yield CollectionField::new('invitations', 'party.crud.field.invitations')
            ->onlyOnDetail()
            ->formatValue(function ($value, Party $entity) use ($adminUrlGenerator)
            {
                if (!$entity || !$entity->getInvitations())
                {
                ...
                foreach ($entity->getInvitations() as $invitation)
                {
                    ...
                    $links[] = sprintf('<a href="%s" target="_self">%s</a>', $url, $invitation);
                }

                return implode('<br>', $links);
            });
        yield CollectionField::new('partyMembers', 'Gäste')
            ->onlyOnDetail()
            ->formatValue(function ($value, Party $entity)
            {
                if (!$entity || $entity->getPartyMembers()->isEmpty())
                {
                    return 'Keine Gäste vorhanden';
                }

                $members = [];
                foreach ($entity->getPartyMembers() as $partyMember)
                {
                    $members[] = $partyMember->getUser();
                }

                return implode('<br>', $members);
            });
    }
